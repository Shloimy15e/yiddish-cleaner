...
const goToPage = (page: number) => {
    router.get(route('imports.index'), { page }, { preserveState: true });
};
</script>
        ...
        <div class="mx-auto flex h-full w-full max-w-7xl flex-1 flex-col gap-6 px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
            ...
            >
                ...
                <template #cell-actions="{ item }">
                    <Link :href="route('imports.show', item.id)" class="text-sm text-primary hover:underline">
                        View
                    </Link>
                </template>

                <template #empty>
                    No import runs yet. <Link :href="route('imports.create')" class="text-primary hover:underline">Start your first import</Link>
                </template>
            </DataTable>
